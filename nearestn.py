...
def is_Nazar(coords):
	"""
	Return a city for Nazar if Nazar coords are applicable
	"""
	if coords is not None:
		Nazar = City("Nazar", coords, 4)
		return Nazar
	else:
		return None
...
Nazar = is_Nazar(Nazar_coords)

points = get_contents('day' + Day + '.txt')

if Nazar is None:
    path = best_route(points)
else:
    trail, dist_N = pathing(Nazar, points)
    path = Path(trail, dist_N)
...
cv2.imwrite('map_adjusted' + '_' + Day + str(DV) + '_' + str(L) +'.png', image)